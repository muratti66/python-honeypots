...
LOG_FILE = "/opt/smtphoneypot/logs/smtphoneypot.log"
CONF_FILE = "/opt/smtphoneypot/config.cfg"

def q_id_generate(size, chars=string.ascii_uppercase + string.digits):
    """
    Queue id generator
...
class HeaderWriter(object):
    ...
    def write_header(self, ip, qid):
        ...
        filename = "/opt/smtphoneypot/headers/" + time + "-" + qid
        data = "smtp" + ";" + time + ";" + ip + ";" + qid
        try:
...
class SMTPCommandWriter(object):
    ...
    def write_commands(self, data, qid):
        ...
        :return: None
        """
        filename = "/opt/smtphoneypot/commands/" + qid
        try:
