...
LOG_FILE = "/opt/proxyhoneypot/logs/proxyhoneypot.log"
CONF_FILE = "/opt/proxyhoneypot/config.cfg"
...
class HeaderWriter(object):
    ...
    def write_header(self, ip, qid):
        ...
        filename = "/opt/proxyhoneypot/headers/" + time + "-" + qid
        data = "proxy;" + time + ";" + ip + ";" + qid
        try:
...
class SMTPCommandWriter(object):
    ...
    def write_commands(self, data, qid):
        ...
        :return: None
        """
        filename = "/opt/proxyhoneypot/commands/" + qid
        try:
...
class DATAWriter(object):
    ...
    def write_data(self, data, qid):
        filename = "/opt/proxyhoneypot/mails/" + qid + ".eml"
        try:
            with open(filename, 'a') as out:
